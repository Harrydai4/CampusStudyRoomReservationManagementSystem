...
public class AppController {
    ...
    public ApiResponse<Void> approve(@PathVariable Long id, @RequestBody(required = false) Map<String, Object> req) {
        app.auditUser(id, true, req == null ? "" : String.valueOf(req.getOrDefault("remark", "")));
        return ApiResponse.ok(null);
    }

    @PostMapping("/admin/users/{id}/reject")
    public ApiResponse<Void> reject(@PathVariable Long id, @RequestBody(required = false) Map<String, Object> req) {
        app.auditUser(id, false, req == null ? "资料不符合要求" : String.valueOf(req.getOrDefault("remark", "资料不符合要求")));
        return ApiResponse.ok(null);
    }
